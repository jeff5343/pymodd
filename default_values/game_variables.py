...
class ItemTypeGroups:
    RANDOMGROUP = Variable('randomgroup', DataType.ITEM_TYPE_GROUP, default_value=[
                           ItemTypes.FROG_TONGUE])


class UnitTypeGroups:
    UNITTYPEGROUP = Variable(
        'unittypegroup', DataType.UNIT_TYPE_GROUP, default_value=[])
...
class Regions:
    TEST = Variable('test', DataType.REGION)
